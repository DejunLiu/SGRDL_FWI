...
% d0 = generate_seismic_data(dom,sources,receivers,all_freqs,m_true,0);
% signanoiseratio = snr(d0, d-d0)
% d0 = generate_seismic_data(dom,sources,receivers,all_freqs,m_true,0);
% norm(d(:,:,1)-d0(:,:,1))/norm(d0(:,:,1))
...
%         dd=m0+W*dm-u_l+q_l;
%         dd=dd(win_inds);
% 		DJ =lambda*(dd) ;	% gradient
% %         DJ=DJ(win_inds);
% 		J = 0.5*lambda*norm(dd).^2;				% value of objective function
		
		DJ =lambda*(m0+W*dm-u_l+q_l) ;	% gradient
        DJ=DJ(win_inds);
		J = 0.5*lambda*norm(m0+W*dm-u_l+q_l).^2;	
